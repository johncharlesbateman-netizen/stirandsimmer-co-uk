...
// Load .env manually so vars are available when the build runs in environments
// that don't auto-populate process.env from .env (e.g. production build).
(() => {
  try {
    ...
    // best-effort; prerender will fail loudly later if vars are still missing
  }
})();
